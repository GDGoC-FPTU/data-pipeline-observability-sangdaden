...
def extract(file_path):
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
        print(f"Extraction complete. Loaded {len(data)} records.")
        return data
...
def validate(data):
    ...
    print(
        f"Validation complete. {len(valid_records)} valid records, {error_count} errors."
    )
...
def transform(data):
    ...
    if df.empty:
        print("Transform complete. Input data is empty.")
        return df
